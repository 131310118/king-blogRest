...
module.exports = function(options) {
  ...
  var middleware = function(req, res, next) {
    ...
    // if (!req.headers.authorization && !req.headers.auth0rization) {
    //   _.set(req, _requestProperty, false);
    //   _.set(req, 'role', false);
    // }
    ...
    async.waterfall([
      ...
      function verifyToken(secret, role, callback) {
        _requestProperty = role;
        jwt.verify(token, secret, options, function(err, decoded) {
          if (err && credentialsRequired) {
            callback(new restify.InvalidCredentialsError(err));
          } else {
      ...
      _.set(req, 'role', _requestProperty);
      next();
    });
  };
